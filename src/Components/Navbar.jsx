...
function Navbar() {
  ...
  const[text,settext] = useState('JAVA/C++');
  const tex=['JAVA/C++','FINANCE','MANAGEMENT','MARKETING','DESIGN','PROGRAMMING','WRITING','ENGINEERING','LAW','IT','DATA SCIENCE','OTHERS']
  const changeText = () => {
    settext(tex[Math.floor(Math.random()*tex.length)]);
  }
useEffect(() => {
...
, []);
  return (
    <div>
      ...
      <nav className='hidden sm:flex justify-between bg-fixed text-black backdrop-blur-xl '>
          <div className='flex justify-center items-center'>
            ...
            <h1 className='text-2xl ml-[2rem] md:text-2xl font-extrabold text-blue-500 font-mono transition ease-in-out'>{text}</h1>
          </div>
      ...
      <nav className='sm:hidden flex justify-between items-center text-black backdrop-blur-xl py-2 px-2'>
        <a href="/">
